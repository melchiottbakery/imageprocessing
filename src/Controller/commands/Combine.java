...
/**
 * This is a command object to execute the Combine command.
 */
public class Combine extends AbstractCommand {
   ...
   *                                  length
   */
  public Combine(String[] command) throws IllegalArgumentException {
    super(command);
    if (command.length != 5) {
  ...
  public void executeCommand(Processor processor, OutputStream print) throws IOException {
    try {
      processor.combine(command[1], command[2], command[3], command[4]);
      print.write(("Combine Operation Success" + System.lineSeparator()).getBytes());
